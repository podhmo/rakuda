...
The `responder.Error` method is also updated to only log errors under specific conditions to reduce noise.

- **5xx Errors:** Always logged.
- **4xx Errors:** Only logged if the logger's level is set to `slog.LevelDebug`.
...
func (r *Responder) Error(w http.ResponseWriter, req *http.Request, statusCode int, err error) {
	...
	if statusCode >= http.StatusInternalServerError || logger.Enabled(ctx, slog.LevelDebug) {
		// ... logging logic with source attribute ...
	}
